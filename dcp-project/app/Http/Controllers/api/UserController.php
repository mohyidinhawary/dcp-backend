...
class UserController extends Controller
{
    ...
    public function getExpertDetails($id){
        $consultant = DB::table('experts')->where('experts.id',$id)->select('id','name','email','phone_number','address','role1','role2','role3','role4','role5','experiences','experience_years')
            ->get();
...
//reservation methodes
    public function AvailableTime($id){
        $user=ExpertAvailability::where('expert_id',$id)->select('day','date','from','to')->get();
        return response()->json([
            "message"=>$user
        ]);
    }    
